...
resetStatus = function() {
     ...
     */
    if (!guest) {
        setStatus("<div class=\"alert alert-info alert-text\">Share the link to your peer to start video chat: <a href=\""+window.location.href+"?peer="+room+"\">"+window.location.href+"?peer="+room+"</a></div>");
    } else {
...
maybeStart = function() {
    if (!started && localStream && channelReady) {
        setStatus("<div class=\"alert alert-info alert-text\">Connecting...</div>");
...
onHangup = function() {
    ...
    pc = null;
    connection.close();
    setStatus("<div class=\"alert alert-danger alert-text\">You hung up the call. Host needs to reload page for new session.</div>");
};
...
onChannelOpened = function() {
    ...
    channelReady = true;
    if(location.search.substring(1,5) == "peer") {
...
onChannelBye = function() {
    ...
    guest = 0;
    started = false;
    setStatus("<div class=\"alert alert-danger alert-text\">Partner has hung up the call. Host needs to reload page for new session.</div>");
};
...
onRemoteStreamAdded = function(event) {
    ...
    setStatus("<div class=\"alert alert-success alert-text\">Video chat is live. <button id=\"hangup\" class=\"btn btn-warning\" onclick=\"onHangup()\">End Call</button></div>");
};
